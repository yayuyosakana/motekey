...
| 不正レスポンス      | APIが期待と異なるJSON構造を返す      | `api-design.md` 3.5 のスキーマ規約で検証。②失敗時は手入力、③失敗時は汎用質問、④失敗時はリトライ導線にフォールバック |
| 不正レスポンス      | APIが期待と異なるJSON構造を返す      | レスポンスのバリデーション。パース失敗時は手入力フォールバック                |
...
正常フロー:
  `mote+AI`タブ押下 → フレーム取得 → Gemini Vision → チャット文脈取得 → Q1〜Q3一括生成 → Q1〜Q3回答(ローカル) → 返信生成
                          ↓失敗             ↓失敗                        ↓失敗                                   ↓失敗
フォールバック:   手入力UI表示 ←── 手入力UI表示                    手入力UI表示                           リトライ
...
| アスクユーザーQ1〜Q3一括生成 | タイムアウト / パースエラー / スキーマ不一致 | リトライボタン / 汎用質問       |
| 返信文生成           | タイムアウト / パースエラー / スキーマ不一致 | リトライボタン                  |
| アスクユーザーQ1〜Q3一括生成 | タイムアウト / パースエラー        | 手入力UI（必要に応じて再試行導線） |
| 返信文生成           | タイムアウト / パースエラー              | リトライボタン（再失敗時は手入力UI） |

---

## 5. Keyboard Runtime状態制御（MVP）

キーボード拡張の状態遷移は、`mote+AI` の質問フロー中断とチップ反映順の扱いを明示的に定義する。

- `mote+AI` タブで質問フロー中（Q1〜Q3）に `キーボード` タブへ切替した場合:
  - 一時状態（質問配列・回答・現在インデックス）を破棄
  - 進行中の in-flight Task を `cancel()` して破棄
  - 画面を通常キーボードへ戻す
- 返信候補の並び替え:
  - MVPでは専用のドラッグ並び替えUIは持たない
  - 実効的な文面順序は「チップをタップした順」
- 送信操作:
  - キーボード拡張側に専用送信ボタンは持たない
  - 既存のメッセージアプリ（LINE）入力欄で手編集し、既存送信UIから送信する
